...
const App: React.FC = () => {
  ...
  const handleRsvpSubmit = useCallback(async (name: string, attending: boolean) => {
    const { error } = await supabase
      .from('rsvps')
      .upsert({ name: name.trim(), attending }, { onConflict: 'name' });
  ...
  const confirmedGuests = responses
    .filter(res => res.attending)
    .map(res => res.name);

  return (
    <div className="min-h-screen bg-[#111827] text-gray-200 flex flex-col items-center p-4 sm:p-6 md:p-8">
      ...
      <main className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col gap-8">
          ...
          <ResponseList confirmedGuests={confirmedGuests} loading={loading} />
          <ActionPanel confirmedGuests={confirmedGuests} />
        </div>
      </main>
    </div>
  );
